...
internals.Utils.prototype.handle_response = function (seneca, data, client_options) {
  ...
  try {
    ...
    seneca.log.error('client', 'callback_error', client_options, data, e.stack || e)
  }
...
internals.Utils.prototype.handle_request = function (seneca, data, listen_options, respond) {
  ...
  if (this._context.options.check.message_loop && _.isArray(data.track)) {
  ...
  var output = internals.Utils.prepareResponse(seneca, data)
  ...
  try {
    ...
    self.catch_act_error(seneca, e, listen_options, data, output)
    respond(output)
  }
}
...
internals.Utils.prototype.make_client = function (context_seneca, make_send, client_options, client_done) {
  ...
  if (pins) {
    ...
    this.make_pinclient(resolvesend, argspatrun, function (err, send) {
      if (err) {
    ...
    this.make_anyclient(client_options, make_send, function (err, send) {
      if (err) {
        return client_done(err)
      }
      client_done(null, send)
...
internals.Utils.prototype.handle_entity = function (raw) {
  ...
  _.each(raw, function (v, k) {
    if (_.isObject(v) && v.entity$) {
      raw[k] = self._context.seneca.make$(v)
    }
  })
  return raw
}
...
internals.Utils.close = function (seneca, closer) {
...
internals.Utils.stringifyJSON = function (seneca, note, obj) {
  if (!obj) {
    return
  }
...
internals.Utils.parseJSON = function (seneca, note, str) {
  if (!str) {
    return
  }
...
internals.Utils.prepareResponse = function (seneca, input) {
  return {
    id: input.id,
    kind: 'res',
    origin: input.origin,
...
internals.Utils.jsonStreamParser = function (seneca) {
  ...
  parser._write = function (data, enc, callback) {
    ...
    while ((endline = str.indexOf('\n', remain)) !== -1) {
      ...
      var outdata = internals.Utils.parseJSON(seneca, 'stream', jsonstr)
...
internals.Utils.error = internals.error

// Needed for transport exports of utils
internals.Utils.prototype.close = internals.Utils.close
internals.Utils.prototype.stringifyJSON = internals.Utils.stringifyJSON
internals.Utils.prototype.parseJSON = internals.Utils.parseJSON
internals.Utils.prototype.prepareResponse = internals.Utils.prepareResponse
