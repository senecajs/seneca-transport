require('seneca')()
  ...
  .add('foo:1,bar:A',function(args,done){done(null,'localA-'+args.bar)})

  .client({pin:{foo:2,bar:'*'}})

  .add('foo:3,bar:C',function(args,done){done(null,'localC-'+args.bar)})

  .client({pin:{foo:4,bar:'*'}})

  .add('foo:5,bar:E',function(args,done){done(null,'localE-'+args.bar)})
